...
int main(int argc, char *argv[]) {
    ...
    while (running) {
        ...
        if (!running) break;
        if (download_aborted || total_bytes == 0) { write_status(status_fd, "IDLE");
        ...
        while (playing) {
            ...
            if (waitpid(mplayer_pid, NULL, WNOHANG) > 0) {
                printf("\n[系统提示] 歌曲播放完毕。\n"); write_status(status_fd, "IDLE"); playing = 0;
            }
        }

        if (mplayer_pid > 0) { kill(mplayer_pid, SIGKILL); waitpid(mplayer_pid, NULL, 0); g_mplayer = 0; }
        unlink(TMP_SONG_FILE);
        printf("[系统提示] 缓存已清理。\n");
    }
